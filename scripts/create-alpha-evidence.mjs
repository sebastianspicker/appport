...
function collect(path) {
  ...
  rejectForbiddenEvidenceInput(path, name);
  const stat = fileSystem.lstatSync(path);
  rejectSymbolicLink(path, stat);
  if (stat.isDirectory()) {
    collectDirectory(path, name);
    return;
  }
  if (!isEvidenceFile(stat, name)) return;
  collectFile(path);
}
function rejectForbiddenEvidenceInput(path, name) {
  if (!forbiddenNames.some((pattern) => pattern.test(name))) return;
  throw new Error(`Forbidden release-evidence input: ${relative(root, path)}`);
}
function rejectSymbolicLink(path, stat) {
  if (!stat.isSymbolicLink()) return;
  throw new Error(
    `Symlinks are not allowed in release evidence: ${relative(root, path)}`,
  );
}
function collectDirectory(path, name) {
  if (excludedDirectories.has(name)) return;
  for (const child of readDirectory(path)) collect(resolve(path, child));
}
function isEvidenceFile(stat, name) {
  return stat.isFile() && !isIgnoredEvidenceFile(name);
}
function isIgnoredEvidenceFile(name) {
  return (
