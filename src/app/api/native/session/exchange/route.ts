...
export async function POST(request: Request) {
  try {
    ...
    if (error instanceof DeviceMatchError) {
      return apiErrorResponse(
        new ApiError(403, error.message, "DEVICE_MATCH_FAILED"),
      );
    }
    if (error instanceof NativeIdentityBindingError) {
      return apiErrorResponse(
        new ApiError(
          403,
          error.message,
