...
function nativeConnectErrorResponse(error: unknown) {
  if (!(error instanceof Error) || !("code" in error)) return null;
  if (error.code === "NATIVE_AUTH_REQUEST_CONFLICT") {
    return apiErrorResponse(
      new ApiError(409, "The native sign-in request cannot be reused."),
    );
  }
  if (error.code === "NATIVE_AUTH_CAPACITY") {
    return apiErrorResponse(
...
async function nativeConnectResponse(url: URL) {
  ...
  const callback = new URL(`http://127.0.0.1:${authorized.loopbackPort}/callback`);
...
export async function GET(request: Request) {
  try {
    ...
    return nativeConnectResponse(new URL(request.url));
  } catch (error) {
    return nativeConnectErrorResponse(error) ?? apiErrorResponse(error);
  }
}
