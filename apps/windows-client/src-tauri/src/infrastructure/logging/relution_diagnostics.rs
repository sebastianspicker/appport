...
fn redact_json(value: &mut Value, retain_number: bool) {
    match value {
        Value::Array(values) => redact_json_array(values),
        Value::Object(values) => redact_json_object(values),
        _ => redact_json_value(value, retain_number),
    }
}

fn redact_json_array(values: &mut [Value]) {
    values
        .iter_mut()
        .for_each(|value| redact_json(value, false));
}

fn redact_json_object(values: &mut serde_json::Map<String, Value>) {
    let mut redacted = serde_json::Map::new();
    for (index, (key, value)) in std::mem::take(values).into_iter().enumerate() {
        redacted.insert(
            redacted_json_key(&key, index),
            redact_json_object_value(key, value),
        );
    }
    *values = redacted;
}

fn redacted_json_key(key: &str, index: usize) -> String {
    if safe_json_key(key) {
        key.to_owned()
    } else {
        format!("field_{index}")
    }
...
fn redact_json_object_value(key: String, mut value: Value) -> Value {
...
fn redact_json_value(value: &mut Value, retain_number: bool) {
    match value {
        ...
        Value::Array(_) | Value::Object(_) => unreachable!("container values are handled first"),
    }
}
