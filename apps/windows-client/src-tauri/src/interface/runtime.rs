//! Local launch-mode and protocol argument interface.
...
pub fn launch_mode(args: &[String]) -> LaunchMode {
    if args
        .iter()
        .skip(1)
        .any(|argument| argument == "--qualification-self-check")
    {
        LaunchMode::QualificationSelfCheck
    } else if args
        .iter()
        .skip(1)
...
mod tests {
    ...
    #[test]
    fn only_exact_self_check_switch_selects_qualification_mode() {
        assert!(matches!(
            launch_mode(&["app".into(), "--qualification-self-check".into()]),
            LaunchMode::QualificationSelfCheck
        ));
        assert!(matches!(
            launch_mode(&["app".into(), "--qualification-self-check=yes".into()]),
            LaunchMode::Foreground
        ));
    }
